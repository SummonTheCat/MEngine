...
public static class ResourceIO
{
    ...
    public static void CacheAnimationCollection(ResConfigModuleAsset asset)
    {
        SysResource sysRes = SysResource.Instance;

        var animConfig = LoadAsset<ResCfgAnimationCollection>(asset.AssetPath);
        if (animConfig == null)
        {
            Debug.LogError($"Failed to load animation collection config: {asset.AssetPath}");
            return;
        }

        // Save ONLY to config cache (no Unity Object resource)
        sysRes.AddToConfigCache(asset.AssetID, animConfig);

        Debug.Log($"Cached AnimationCollection Config: {asset.AssetID} (Animations={animConfig.AnimationScenes?.Length ?? 0})");
    }
